  ...
  describe('Ref', () => {
    it('Should parse a ref', () => {
      expect(ParseProperty('ref Test("id", "id")')).to.deep.equal(
        new SpecProperty(constants.ReferenceModel, {
          foreignProperty: 'id',
          localProperty: 'id',
          modelName: 'Test'
        })
  ...
  describe('Import', () => {
    it('Should parse an import', () => {
      expect(ParseProperty('import Test')).to.deep.equal(
        new SpecProperty(constants.ImportModel, { modelName: 'Test' })
      )
    })
    it('Should parse a nullable import', () => {
      expect(ParseProperty('import Test?')).to.deep.equal(
        new SpecProperty(constants.ImportModel, {
          modelName: 'Test',
          nullable: true
        })
      )
  ...
  describe('Integer', () => {
    ...
    it('Should parse with autogenerate options', () => {
      expect(ParseProperty('integer auto(1,1)')).to.deep.equal(
        new SpecProperty('integer', {
          autogenerateOptions: new IntegerAutogenerateOptions(1, 1)
        })
      )
    })
