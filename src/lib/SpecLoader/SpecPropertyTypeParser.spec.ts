...
describe('SpecPropertyTypeParser', () => {
  ...
  describe('Integer', () => {
    ...
    it('Should parse a nullable integer', () => {
      expect(ParseProperty('integer?')).to.deep.equal(new SpecProperty('integer', { nullable: true }))
    })
    it('Should parse as an array', () => {
      expect(ParseProperty('integer[]')).to.deep.equal(new SpecProperty('integer', { isArray: true }))
    })
    it('Should parse as a nullable array', () => {
      expect(ParseProperty('integer[]?')).to.deep.equal(new SpecProperty('integer', { isArray: true, nullable: true }))
    })
    it('Should parse with a default value', () => {
      expect(ParseProperty('integer default(5)')).to.deep.equal(new SpecProperty('integer', { defaultValue: 5 }))
    })
    it('Should parse with a primary key', () => {
      expect(ParseProperty('integer primary')).to.deep.equal(new SpecProperty('integer', { primaryKey: true }))
    })
    it('Should parse with a min value', () => {
      expect(ParseProperty('integer min(0)')).to.deep.equal(new SpecProperty('integer', { minValue: 0 }))
    })
    ...
    it('Should parse with autogenerate options', () => {
      expect(ParseProperty('integer auto(1,1)')).to.deep.equal(new SpecProperty('integer', { autogenerateOptions: new IntegerAutogenerateOptions(1, 1) }))
    })
  ...
  describe('Boolean', () => {
    ...
    it('Should parse a nullable boolean', () => {
      expect(ParseProperty('boolean?')).to.deep.equal(new SpecProperty('boolean', { nullable: true }))
    })
    it('Should parse as an array', () => {
      expect(ParseProperty('boolean[]')).to.deep.equal(new SpecProperty('boolean', { isArray: true }))
    })
    it('Should parse as a nullable array', () => {
      expect(ParseProperty('boolean[]?')).to.deep.equal(new SpecProperty('boolean', { isArray: true, nullable: true }))
    })
    it('Should parse with a default value', () => {
      expect(ParseProperty('boolean default(true)')).to.deep.equal(new SpecProperty('boolean', { defaultValue: true }))
    })
    it('Should parse with a delete flag', () => {
      expect(ParseProperty('boolean delete')).to.deep.equal(new SpecProperty('boolean', { deleteFlag: true }))
    })
  ...
  describe('Decimal', () => {
    ...
    it('Should parse a nullable decimal', () => {
      expect(ParseProperty('decimal?')).to.deep.equal(new SpecProperty('decimal', { nullable: true }))
    })
    it('Should parse as an array', () => {
    ...
    it('Should parse with a nullable with precision and scale', () => {
      expect(ParseProperty('decimal(12,2)?')).to.deep.equal(new SpecProperty('decimal', { nullable: true, precision: 12, scale: 2 }))
    })
    it('Should parse an array with precision and scale', () => {
      expect(ParseProperty('decimal(12,2)[]')).to.deep.equal(new SpecProperty('decimal', { isArray: true, precision: 12, scale: 2 }))
    })
    it('Should parse a nullable array with precision and scale', () => {
      expect(ParseProperty('decimal(12,2)[]?')).to.deep.equal(new SpecProperty('decimal', { isArray: true, nullable: true, precision: 12, scale: 2 }))
    })
    it('Should parse with a default value', () => {
      expect(ParseProperty('decimal default(2.5)')).to.deep.equal(new SpecProperty('decimal', { defaultValue: 2.5 }))
    })
    it('Should parse with primary key', () => {
      expect(ParseProperty('decimal primary')).to.deep.equal(new SpecProperty('decimal', { primaryKey: true }))
    })
    ...
    it('Should parse with a min value', () => {
      expect(ParseProperty('decimal min(-1.5)')).to.deep.equal(new SpecProperty('decimal', { minValue: -1.5 }))
    })
    it('Should parse with a min and max value', () => {
  ...
  describe('DateTime', () => {
    ...
    it('Should parse a nullable datetime', () => {
      expect(ParseProperty('datetime?')).to.deep.equal(new SpecProperty('datetime', { nullable: true }))
    })
    it('Should parse an array of datetime', () => {
      expect(ParseProperty('datetime[]')).to.deep.equal(new SpecProperty('datetime', { isArray: true }))
    })
    it('Should parse a nullable array of datetime', () => {
      expect(ParseProperty('datetime[]?')).to.deep.equal(new SpecProperty('datetime', { isArray: true, nullable: true }))
    })
    it('Should parse with default of now', () => {
      validNowTokens.forEach((nowToken) => {
        expect(ParseProperty(`datetime default(${nowToken})`)).to.deep.equal(new SpecProperty('datetime', { defaultValue: 'DateTimeNow' }), `Test default value: ${nowToken}`)
      })
    })
    it('Should parse with default value', () => {
      validDates.forEach((testDate) => {
        expect(ParseProperty(`datetime default(${testDate})`)).to.deep.equal(new SpecProperty('datetime', { defaultValue: new Date(testDate) }), `Test default value: ${testDate}`)
      })
    })
    it('Should parse with max value', () => {
      validDates.forEach((testDate) => {
        expect(ParseProperty(`datetime max(${testDate})`)).to.deep.equal(new SpecProperty('datetime', { maxValue: new Date(testDate) }), `Test default value: ${testDate}`)
      })
    })
    it('Should parse with min value', () => {
      validDates.forEach((testDate) => {
        expect(ParseProperty(`datetime min(${testDate})`)).to.deep.equal(new SpecProperty('datetime', { minValue: new Date(testDate) }), `Test default value: ${testDate}`)
      })
    })
    it('Should parse with min and max value', () => {
      validDates.forEach((testDate) => {
        expect(ParseProperty(`datetime min(${testDate}) max(${testDate})`)).to.deep.equal(new SpecProperty('datetime', { maxValue: new Date(testDate), minValue: new Date(testDate) }), `Test default value: ${testDate}`)
      })
    })
  ...
  describe('String', () => {
    ...
    it('Should parse a string with length', () => {
      expect(ParseProperty('string(100)?')).to.deep.equal(new SpecProperty('string', { length: 100, nullable: true }))
    })
    it('Should parse with primary key', () => {
      expect(ParseProperty('string primary')).to.deep.equal(new SpecProperty('string', { primaryKey: true }))
    })
    it('Should parse with unique', () => {
      expect(ParseProperty('string unique')).to.deep.equal(new SpecProperty('string', { unique: true }))
    })
    it('Should parse with default value', () => {
      expect(ParseProperty('string default("Testing")')).to.deep.equal(new SpecProperty('string', { defaultValue: 'Testing' }))
      expect(ParseProperty('string default("")')).to.deep.equal(new SpecProperty('string', { defaultValue: '' }))
      expect(ParseProperty('string default(\'Testing\')')).to.deep.equal(new SpecProperty('string', { defaultValue: 'Testing' }))
      expect(ParseProperty('string default(\'\')')).to.deep.equal(new SpecProperty('string', { defaultValue: '' }))
    })
    it('Should parse allowed values', () => {
      expect(ParseProperty('string allow("Value 1", "Value 2", "Value 3")')).to.deep.equal(new SpecProperty('string', { allowedValues: ['Value 1', 'Value 2', 'Value 3'] }))
      expect(ParseProperty('string allow(\'Value 1\', \'Value 2\', \'Value 3\')')).to.deep.equal(new SpecProperty('string', { allowedValues: ['Value 1', 'Value 2', 'Value 3'] }))
    })
  ...
  describe('UniqueIdentifier', () => {
    ...
    it('Should parse a nullable uniqueidentifier', () => {
      expect(ParseProperty('uniqueidentifier?')).to.deep.equal(new SpecProperty('uniqueidentifier', { nullable: true }))
    })
    it('Should parse an array of uniqueidentifier', () => {
      expect(ParseProperty('uniqueidentifier[]')).to.deep.equal(new SpecProperty('uniqueidentifier', { isArray: true }))
    })
    it('Should parse a nullable array of uniqueidentifier', () => {
      expect(ParseProperty('uniqueidentifier[]?')).to.deep.equal(new SpecProperty('uniqueidentifier', { isArray: true, nullable: true }))
    })
    it('Should parse with primary key', () => {
      expect(ParseProperty('uniqueidentifier primary')).to.deep.equal(new SpecProperty('uniqueidentifier', { primaryKey: true }))
    })
    it('Should parse with unique', () => {
      expect(ParseProperty('uniqueidentifier unique')).to.deep.equal(new SpecProperty('uniqueidentifier', { unique: true }))
    })
  })
})
